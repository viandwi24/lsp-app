@extends('layouts.dashboard', ['title' => 'Asesor \ Skema \ '. $skema->judul .' \ FORMULIR UMPAN BALIK'])
...
@section('content')
    ...
    <x-dashboard-content>
        <div class="row">
            <div class="col-12">
                <x-dashboard-card title="Edit Formulir" classBody="card-table">
                    <x-slot name="heading">
                    ...
                    <!-- Body -->
                    <form method="POST" action="{{ route('asesor.skema.umpanbalik.update', [$skema->id]) }}" class="form" id="form">
                        @csrf
                        @method('put')
...
@push('js')
    <script>
        var vm = new Vue({
            el: '#app',
            data: {
                pertanyaans: @JSON($skema->umpanbalik->pertanyaan)
            },
            mounted() {
