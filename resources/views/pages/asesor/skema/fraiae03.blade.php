@extends('layouts.dashboard', ['title' => 'Asesor \ Skema \ '. $skema->judul .' \ FR-AI-AE-03'])
        ...
        <div class="row">
            <div class="col-12">
                <x-dashboard-card title="Edit Formulir FR-AI-AE-03" classBody="card-table">
                    ...
                    <!-- Body -->
                    <form method="POST" action="{{ route('asesor.skema.fraiae03.update', [$skema->id]) }}" class="form" id="form">
                        @csrf
                        @method('put')
...
@push('js')
    <script>
        var vm = new Vue({
            el: '#app',
            data: {
                pertanyaans: @JSON($skema->fraiae03->pertanyaan)
            },
            mounted() {
