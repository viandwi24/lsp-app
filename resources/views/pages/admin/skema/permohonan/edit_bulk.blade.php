            ...
            <div class="col-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body card-dashboard">
                        <form method="POST" action="{{ url()->route('admin.skema.permohonan.update', [$skema->id, $permohonans->pluck('id')->join(',')]) }}" class="form">
                            @method('put')
                            @csrf
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-1">
                                        <input readonly value="{{ $no . ' - ' . $permohonan->asesi->nama }}" type="text" class="form-control">
                                    </div>
                                    @php $no++ @endphp
                                    @endforeach
                                </div>
                            ...
                            <div class="form-group row">
                                <div class="col-md-6 offset-md-4">
                                    <a href="{{ route('admin.skema.permohonan.index', [$skema->id, $permohonans->pluck('id')->join(',')]) }}" class="btn btn-warning">
                ...
                <div id="accordion">
                    @php $no = 1; @endphp
                    @foreach ($permohonans as $permohonan)
                        @php $data = $permohonan->data @endphp
                        <div class="card m-0">
                            <div class="card-header" id="asesi-{{ $no }}">
                                <h5 class="mb-0">
                                <button class="btn btn-link" data-toggle="collapse" data-target="#collapse-asesi-{{ $no }}" aria-expanded="true" aria-controls="collapse-asesi-{{ $no }}">
                                    {{ $no }} - {{ $data->data_diri->nama }}
                                </button>
                                </h5>
                            </div>
                            <div id="collapse-asesi-{{ $no }}" class="collapse" aria-labelledby="asesi-{{ $no }}" data-parent="#accordion">
                                <div class="card-body">
                                    <div class="card-body card-dashboard">
                                        <div class="form">
                                            <div class="form-group row">
