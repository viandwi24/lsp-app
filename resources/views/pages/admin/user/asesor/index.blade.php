@extends('layouts.dashboard', ['title' => 'Admin \ User \ Asesor'])
...
@section('content')
    <!-- content-header -->
    <x-dashboard-content-header title="Manajemen Asesor" :breadcrumb="$breadcrumb" />
    
    <!-- content -->
    <x-dashboard-content>
        <div class="row">
            <div class="col-12">
                <x-dashboard-card title="Tempat Uji Kompetensi" classBody="card-table">
                    <x-slot name="heading">
                        ...
                        <li><a class="btn btn-sm btn-success" href="{{ route('admin.user.asesor.create') }}"><i class="ft-plus"></i> Tambah</a></li>
    ...
    <script>
        var vm = new Vue({
            ...
            methods: {
                loadTable() {
                    ...
                    this.datatable = $('#table').DataTable( {
                        ajax: "{{ url()->route('admin.user.asesor.index') }}",
                        processing: true,
                        order: [[1, 'asc']],
                        ...
                        columns: [
                            ...
                            { data: 'email' },
                            {
                                ...
                                render: (data, type, row) => `
                                    <div>
                                        <a href="{{ url()->route('admin.user.asesor.index') . '/' }}`+data.id+`/edit" class="btn btn-sm btn-warning"><i class="ft-edit"></i></a>
                                        <form method="post" action="{{ url()->route('admin.user.asesor.index') . '/' }}`+data.id+`" style="display: inline-block;">
                                            @method('delete')
                                            @csrf
            ...
            mounted() {
                ...
                $('#bulkDelete').on('click', () => {
                    var url = '{{ url()->route("admin.user.asesor.index") }}/' + bulkSelectedItem
